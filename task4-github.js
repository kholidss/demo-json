...
function showNullPhone() {
  let no1 = data.map((x) => x.profile).filter((a) => a.phones.length == 0);
  console.log({ no1 });
}
// showNullPhone();
...
function showDate(inputDate) {
  let no7 = data
    ...
    .flat()
    .filter((b) => {
      let dateData = b.published_at;
      let convDate = new Date(dateData);
      let convInputDate = new Date(inputDate);

      return convDate < convInputDate;
    });
  console.log(no7);
}
showDate("2019-08");
